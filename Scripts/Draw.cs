...
public class Draw : MonoBehaviour
{
    ...
    void Start()
    {
        material = Resources.Load<Material>("1");
        ...
        if ((vectors.Count() - vectosRemoveRepeat.Count()) < 10)
        {
                try
                {
                    ...
                    string name = "儿综-房间体块-"+ GetName(roomBoundarys[i].Name) + "[" + roomBoundarys[i].ElementId + "]";
                    CreateMesh(vectors, roomBoundarys[i].Height, 10,name);
                ...
                {
                    Debug.Log(roomBoundarys[i].ElementId);
                }
        }
        ...
        else
        {
                try
                {
                    ...
                    string name = "儿综-房间体块-" + GetName(roomBoundarys[i].Name) + "["+roomBoundarys[i].ElementId+"]";
                    CreateMesh(vectosRemoveRepeat, roomBoundarys[i].Height, 10,name);
    ...
    public static List<RoomBoundary> Convert(string path)
    {
        json = File.ReadAllText(Application.dataPath + "\\Resources\\RoomBoundary.json");

        roomBoundarys = JsonConvert.DeserializeObject<List<RoomBoundary>>(json);

        return roomBoundarys;
    ...
    public static void CreateRoom(List<RoomBoundary> roomBoundaries)
    {
        ...
        for (int i = 0; i < roomBoundarys.Count(); i++)
        {
            ...
            if ((vectors.Count() - vectosRemoveRepeat.Count()) < 10)
            {
                ...
                Debug.Log(vectors.Count() + "炸弹" + roomBoundarys[i].ElementId);
                string name = "儿综-房间体块-" + GetName(roomBoundarys[i].Name) + "[" + roomBoundarys[i].ElementId + "]";
                CreateMesh(vectors,roomBoundarys[i].Height, thickness,name);
                //测试方法
                //GeneratePoints(vectors);
                //DrawLine(vectors.ToArray(), line);
                //GeneratePoints(vectors);
            ...
            else
            {
                Debug.Log(vectosRemoveRepeat.Count() + "除重炸弹" + roomBoundarys[i].ElementId);
                string name = "儿综-房间体块-" + GetName(roomBoundarys[i].Name) + "[" + roomBoundarys[i].ElementId + "]";
                CreateMesh(vectosRemoveRepeat, roomBoundarys[i].Height, thickness,name);
                //测试方法
                //GeneratePoints(vectosRemoveRepeat);
                //DrawLine(vectors.ToArray(), line);
            }
    ...
    {
        string Name="";
        var matchXF = Regex.Match(name, @"-([0-9]+[FL]+M*)");
        var matchFX = Regex.Match(name, @"-([FL]+[0-9]+M*)");
        var matchBB = Regex.Match(name, @"-([B][0-9][F])");
        var matchB = Regex.Match(name, @"-([B][0-9])");
        var matchMF = Regex.Match(name, @"-([0-9][M][F])");
        var matchGF = Regex.Match(name, @"-([0-9]+F#[A-Z])");
        if (matchGF.Success)
        {
            Name = matchGF.Groups[1].Value;
        }
        else if (matchBB.Success)
        {
            Name = matchBB.Groups[1].Value;
        }
        else if (matchFX.Success)
        {
            Name = matchFX.Groups[1].Value;
        }
        else if (matchB.Success)
        {
            Name = matchB.Groups[1].Value;
        }
        else if (matchXF.Success)
        {
            Name = matchXF.Groups[1].Value;
        }
        else if (matchMF.Success)
        {
            Name = matchMF.Groups[1].Value;
        }
        return Name;
    }
    ...
    public  static void CreateMesh(List<Vector3> points,double elevation,double height,string name)
    {
        List<Vector3> points1 = points;

        List<Vector3> points2 = points1.Select(p => p = new Vector3(p.x, p.y + 1000, p.z)).ToList();

        GameObject M = new GameObject("下面");
        GameObject M2 = new GameObject("上面");
        ...
        int[] bottomTriangles = M.GetComponent<MeshFilter>().mesh.triangles;
        for (int i = 0; i < bottomTriangles.Length; i += 3)
        {
            int t = bottomTriangles[i];
            bottomTriangles[i] = bottomTriangles[i + 2];
            bottomTriangles[i + 2] = t;
        }
        M.GetComponent<MeshFilter>().mesh.triangles = bottomTriangles;
        M.transform.position = new Vector3(M.transform.position.x,(float)elevation/1000,M.transform.position.z);
        ...
        int[] triangles = M.GetComponent<MeshFilter>().mesh.triangles;
        ...
        M2.transform.position = new Vector3(M.transform.position.x, M.transform.position.y + (float)height, M.transform.position.z);
        ...
        var CeMian=MergeMesh(M.GetComponent<MeshFilter>().mesh,elevation,height);
    ...
    /// <returns></returns>
    public static GameObject MergeMesh(Mesh mesh1,double elevation,double height)
    {
        Vector3[] points1 = mesh1.vertices;
        points1 = points1.Select(x => x = new Vector3(x.x, (float)elevation/1000, x.z)).ToArray();
        Vector3[] points2 = points1.Select(x => x = new Vector3(x.x, x.y + (float)height, x.z)).ToArray();
